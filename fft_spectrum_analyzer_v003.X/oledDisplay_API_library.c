...
void OLED_Update( void )
{
  ...
  for (i=0; i<(SSD1306_LCDWIDTH*SSD1306_LCDHEIGHT/8); i++)
  {
    ...
    I2C_Send(SSD1306_ADDR << 1); // slave address and R/nW
...
static void ssd1306_command( uint8_t command )
{
  ...
  I2C_Start(); //send start command
  I2C_Send(SSD1306_ADDR << 1); //transmit slave address and R/nW
  I2C_Send(control); //transmit control byte
...
static void ssd1306_data( uint8_t value )
{
  ...
  I2C_Send(SSD1306_ADDR << 1); //transmit slave address and R/nW
  I2C_Send(control); //transmit control byte
  I2C_Send(value); //transmit data byte
  I2C_Stop(); //send stop command
}
 ...
 * 
 * @param inputArray[]: an array of size inputSize(500) with a linear bin width of inputBin(6.4 Hz), total range 3.2 kHz
...
void resize_freq_array(int inputArray[], int outputArray[], int fundamental){
    ...
    float inputBin = 6.4; //bin width of the input array
